...
%%
p1 = plot(Formula_Steady_State_20s.latacc.Data, Formula_Steady_State_20s.SWA.Data);
hold on
...
% acceleration) the formula vehicle shows a gradient of ~5.34  deg / ms^-2, while 
% the road vehicle shows a much shallower response of ~2.13 deg / ms^-2. The Formula 
% vehicle response is more than double that of the road vehicle.
% 
% Note: The constant speed of 120km/h is a reltively low speed for a performance 
% vehicle, wheras it is at the higher end of typical operating speeds for a road 
% going vehicle.
%% Calculating the understeer gradient of each vehicle.
% For the Formula Vehicle SWA gradient of 5.34 deg/ms^-2 and Road Vehicle SWA 
% gradient 2.13 deg/ms^-2, we can determine the understeer gradient:
% 
% K_us = [(dSWA/dLatAcc) / SR] - [(L/V^2) .* 180/pi]

Kus_FormulaV = (2.13 .* (1/10)) - ((3.5 / ((120/3.6)^2)) .* (180/pi))
Kus_RoadV = (5.34 .* (1/20)) - ((2.8 / ((120/3.6)^2)) .* (180/pi))
%% 
% With the Formula Vehicle having an understeer ratio of 0.0325 and the Road 
% Vehicle having an understeer ratio of 0.1226, we can see that both vehicles 
...
p3 = plot(Formula_Steady_State_20s.latacc.Data, Formula_Steady_State_20s.sideslip.Data);
hold on
p4 = plot(Road_Vehicle_Steady_State_40s.latacc.Data, Road_Vehicle_Steady_State_40s.sideslip.Data);
...
% gradient of ~-0.0345 deg / ms^-2. This means that the side slip increases in 
% magnitude by 0.03 degrees for each unit m/s^2 increase of lateral acceleration. 
% In comparison, the road vehicle shows a steep increase in side slip angle magnitude 
% for each unit lateral acceleration, with a gradient of ~-0.6790 deg / ms^-2. 
% Recalling that the vehicle sideslip angle is the angle offset between the direction 
% of the wheel and the direction of motion of the vehicle, we expect this angle 
% to be greater in an understeer scenario compared to a balanced or neutral vehicle 
...
% 
% The road vehicle requires ~20 times more side slip to generate the same level 
...
% 
% The formula vehicle is critically dampes, showing no overshoot in lateral 
% acceleration.
...
% vehicle yaw rate overshoot only ~1.16% compared to the road vehicle's ~13.7% 
% it is clear that the performance of the formula vehicle is greater. This is 
% further reflected by the time taken for the vehicle response to reach its steady 
% state condition being significantly greater for the road vehicle compared to 
% the formula vehicle.
...
%% 
% The response of the formula vehicle is greater than that of the road vehicle, 
% with steady state conditions achieved much faster and with significantly less 
% overshoot.